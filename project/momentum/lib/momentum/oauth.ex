defmodule Momentum.OAuth do
  ...
  @spec build_request(token_params :: map(), url_verb()) :: {header(), params()}
  def build_request(rt_params, url_verb) do
  ...
  defp parse_query_params(params), do: params |> URI.query_decoder() |> Enum.to_list()

  defp nonce(), do: 24 |> :crypto.strong_rand_bytes() |> Base.encode64()
  ...
  defp maybe_put_token(params, value), do: [{"oauth_token", value} | params]
  ...
  defp percent_encode(other), do: other |> to_string() |> URI.encode(&URI.char_unreserved?/1)
end
